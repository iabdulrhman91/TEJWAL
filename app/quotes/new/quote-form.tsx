...
import { Plus, Trash2, Save, Plane, Building2, Calendar, MapPin, HelpCircle, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react'
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border">
                ...
                <div className="space-y-4">
                    {flights.map(flight => (
                        <div key={flight.localId} className="bg-gray-50 p-4 rounded-xl relative border border-gray-200">
                            <button type="button" onClick={() => removeFlight(flight.localId)} className="absolute top-4 left-4 text-gray-400 hover:text-red-500"><Trash2 size={18} /></button>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    {flight.showTransit && (
                                        <div className="mt-2 pt-2 border-t border-gray-100">
                                            <div className="flex justify-between items-center mb-1">
                                                <label className="text-[10px] text-gray-500 font-bold">المدة (ساعة)</label>
                                                <button type="button" onClick={() => updateFlight(flight.localId, { stopover: '', stopoverTime: 0, showTransit: false })} className="text-red-400 hover:text-red-500"><Trash2 size={12} /></button>
                                            </div>
                                            <NumberInput
                                ...
                                <div className="md:col-span-1"><label className="text-xs text-gray-500">الخطوط</label><AirlineAutocomplete value={flight.airline} onChange={val => updateFlight(flight.localId, 'airline', val)} placeholder="اختر شركة الطيران..." /></div>
                                <div className="md:col-span-1"><label className="text-xs text-gray-500">الوزن (كجم)</label><input type="text" inputMode="numeric" placeholder="مثلاً: 25" value={flight.weight || ''} onChange={e => updateFlight(flight.localId, 'weight', e.target.value.replace(/[^0-9]/g, ''))} className="w-full p-2 border rounded-lg bg-white num-en font-bold" /></div>
                                <div className="md:col-span-1">
                                    <label className="text-xs text-gray-500 mb-2 block">نوع الرحلة</label>
                                    <label className="flex items-center gap-2 cursor-pointer bg-white p-2 border rounded-lg hover:bg-gray-50 transition h-[42px]">
                                        <input type="checkbox" checked={flight.flightType === 'RoundTrip'} onChange={e => updateFlight(flight.localId, 'flightType', e.target.checked ? 'RoundTrip' : 'OneWay')} className="w-4 h-4 text-blue-600 rounded" />
                                        <span className="text-sm font-medium">ذهاب وعودة</span>
                                    </label>
                                </div>
                                {flight.flightType === 'RoundTrip' && (
                                    <div className="md:col-span-4 grid grid-cols-1 md:grid-cols-4 gap-4 bg-orange-50/30 p-4 rounded-xl border border-orange-100 mt-2">
                                        <div className="md:col-span-4 text-orange-600 font-bold text-xs border-b border-orange-100 pb-2">تفاصيل رحلة العودة</div>
                                        <div><label className="text-[10px] text-gray-500">من</label><AirportAutocomplete value={flight.returnFrom || ''} onChange={val => updateFlight(flight.localId, 'returnFrom', val)} placeholder="المغادرة..." /></div>
                                        <div>
                                            <label className="text-[10px] text-gray-500">إلى</label>
                                            <AirportAutocomplete value={flight.returnTo || ''} onChange={val => updateFlight(flight.localId, 'returnTo', val)} placeholder="الوصول..." />

                                            {flight.showReturnTransit ? (
                                                <div className="mt-2 pt-2 border-t border-orange-100">
                                                    <div className="flex justify-between items-center mb-1">
                                                        <label className="text-[9px] text-orange-600 font-bold">ترانزيت العودة</label>
                                                    </div>
                                                ...
                                                <button type="button" onClick={() => updateFlight(flight.localId, 'showReturnTransit', true)} className="text-[9px] text-blue-600 font-bold mt-1 flex items-center gap-0.5 hover:underline transition-all">
                                                    <Plus size={10} /> إضافة ترانزيت
                                                </button>
                                            )}
                                        </div>
                                        <div>
                                            <label className="text-[10px] text-gray-500">المغادرة</label>
                                            <CustomDatePicker
                                            ...
                                            {flight.showReturnTransit && (
                                                <div className="mt-2 pt-2 border-t border-orange-100">
                                                    <div className="flex justify-between items-center mb-1">
                                                        <label className="text-[10px] text-gray-500 font-bold">المدة (ساعة)</label>
                                                        <button type="button" onClick={() => updateFlight(flight.localId, { returnStopover: '', returnStopoverTime: 0, showReturnTransit: false })} className="text-red-400 hover:text-red-500"><Trash2 size={10} /></button>
                                                    </div>
                                                    <NumberInput
                                        ...
                                        <div>
                                            <label className="text-[10px] text-gray-500">الوصول</label>
                                            <CustomDatePicker
                                        ...
                                        <div className="md:col-span-1"><label className="text-[10px] text-gray-500">الخطوط</label><AirlineAutocomplete value={flight.returnAirline || ''} onChange={val => updateFlight(flight.localId, 'returnAirline', val)} placeholder="شركة الطيران..." /></div>
                                        <div className="md:col-span-1"><label className="text-[10px] text-gray-500">الوزن (كجم)</label><input type="text" inputMode="numeric" placeholder="مثلاً: 25" value={flight.returnWeight || ''} onChange={e => updateFlight(flight.localId, 'returnWeight', e.target.value.replace(/[^0-9]/g, ''))} className="w-full p-2 border rounded-lg bg-white bg-orange-50/50 num-en font-bold" /></div>
                                    </div>
                                )}
                                <div>
                                    ...
                                    >
                                        ...
                                        {flightSuppliers.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
                                    </select>
                                </div>
                                <div><label className="text-xs text-orange-600 font-bold">سعر التكلفة</label><input type="number" lang="en" dir="ltr" value={flight.costPrice || ''} onChange={e => updateFlight(flight.localId, 'costPrice', parseFloat(e.target.value) || 0)} className="w-full p-2 border-orange-200 border rounded font-bold text-orange-700 no-spin" /></div>
                                <div><label className="text-xs text-gray-500">التذاكر</label><NumberInput value={flight.ticketCount} onChange={val => updateFlight(flight.localId, 'ticketCount', val)} /></div>
                            </div>
                        </div>
                    ))}
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button type="button" onClick={addHotel} className="text-purple-600 hover:bg-purple-50 px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm font-medium font-bold transition"><Plus size={18} /> إضافة فندق</button>
                </div>
                <div className="space-y-4">
                    {hotels.map(hotel => (
                        <div key={hotel.localId} className="bg-gray-50 p-4 rounded-xl relative border border-gray-200">
